...
$(window).on('scroll',function(){counter();});$(document).ready(function(){$('.has-animation').each(function(index){$(this).delay($(this).data('delay')).queue(function(){$(this).addClass('animate-in');});});});$(document).ready(function(){$('#joinUsForm').submit(function(event){var formData={'name':$('input[name=name]').val(),'mail':$('input[name=mail]').val(),'reason':$('input[name=message]').val()};$.ajax({type:'POST',url:'https://rbb-data-api.cisco.com/api/v1/rbb/join-us',headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',},data:JSON.stringify(formData),dataType:'json',encode:true}).done(function(data){console.log(data);});event.preventDefault();});});$(document).ready(function(){$('#loginForm').submit(function(event){var formData={'userName':$('input[name=username]').val(),'password':$('input[name=password]').val()};console.log(formData);$.ajax({type:'POST',url:'https://rbb-data-api.cisco.com/api/v1/rbb/member-login',headers:{'Content-Type':'application/json','Access-Control-Allow-Origin':'*',},data:JSON.stringify(formData),dataType:'json',encode:true}).done(function(data){console.log(data);});event.preventDefault();});});})(jQuery);